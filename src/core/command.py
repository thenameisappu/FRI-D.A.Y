...
def execute_command(command, background=False):
    ...
    if not background and any(keyword in command for keyword in ['record screen', 'screen record']):
         task_manager.run_in_background(execute_command, (command,), {"background": True})
         return True
        ...
        if any(word in command for word in ['search file', 'copy file', 'move file', 'delete file']):
            ...
            if city:
                ...
                if weather:  # ← add this check
                    speak(f"Current weather in {city}: {weather['temperature']}°C, "
                        f"{weather['description']}, humidity {weather['humidity']}%, "
                        f"wind speed {weather['wind_speed']} meters per second")
            ...
            if not background: # This shouldn't be reached if the check above works, but for safety
                task_manager.run_in_background(execute_command, (command,), {"background": True})
                return True
            ...
            try:
                for proc in psutil.process_iter(['name']):
                    ...
                    if proc_name and any(player in proc_name.lower() for player in known_media_players):
